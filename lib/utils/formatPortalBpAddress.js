...
/** Decode common HTML ampersand entities left in SAP/portal address strings. */
function decodeAddressHtmlEntities(value) {
  return value
    .replace(/&amp;/gi, '&')
    .replace(/&#38;/g, '&')
    .replace(/&#x26;/gi, '&');
...
export function sanitizeAddressPart(value) {
  ...
  const s = decodeAddressHtmlEntities(String(value).trim());
  if (!s) return '';
