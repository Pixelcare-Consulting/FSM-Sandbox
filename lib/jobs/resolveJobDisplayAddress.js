...
export function resolveJobDisplayAddress(job, context = {}) {
  ...
  if (tagAddress) {
    const decoded = sanitizeAddressPart(tagAddress.replace(/\s+/g, ' ').trim());
    if (decoded) return decoded;
  }

  const locationName = sanitizeAddressPart(
    job.location?.location_name || job.location?.locationName || ''
  );
  ...
  const scheduleAddress = sanitizeAddressPart(
    context.scheduleAddress ?? job.scheduleAddress ?? ''
  );
